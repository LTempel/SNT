 ...
 */
@Plugin(type = Command.class, visible = false, label="Match Path(s)...", initializer = "init")
public class PathMatcherCmd extends CommonDynamicCmd {

	@Parameter(label = "<HTML><b>Frame Range Criteria:", persist = false, required = false, visibility = ItemVisibility.MESSAGE)
	...
	@Parameter(label = "<HTML>&nbsp;<br><b>Location Criteria for Starting Node:", persist = false, required = false, visibility = ItemVisibility.MESSAGE)
	...
	public void run() {
		...
		while (!mPaths.isEmpty()) {
			...
			for (final MatchingPath hit : pathsWithCommonRoot) {
				hit.assignID(counter++);
			}
			mPaths.removeAll(pathsWithCommonRoot);
		}
		if (counter == 1) {
			msg("All paths share the same time point and were assigned a common group tag.", "Matching Completed");
		} else {
			final String timePointsParsed = (timePoints == null) ? "all" : String.valueOf(timePoints.size());
			msg(String.format("%d paths assigned to %d group(s) across %s timepoints.", paths.size(), counter,
